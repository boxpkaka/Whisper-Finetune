...
for step, batch in enumerate(tqdm(eval_dataloader)):
    with torch.cuda.amp.autocast():
        with torch.no_grad():
            ...
            # 将预测和实际的token转换为文本
            decoded_preds = processor.tokenizer.batch_decode(generated_tokens, skip_special_tokens=False)
            decoded_labels = processor.tokenizer.batch_decode(labels, skip_special_tokens=False)
